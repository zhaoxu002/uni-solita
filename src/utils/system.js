export default function getScreenInfo () {
 ...
 const modelmes = result.model
  ...
  const capsuleInfo = wx.getMenuButtonBoundingClientRect()
   ...
   // 可视区域高度 - 适配横竖屏场景
   const autoScreenHeight = Math.max(result.screenHeight, result.screenWidth)
   const height = Math.max(safeArea.height, safeArea.width)
   // 状态栏高度
   const statusBarHeight = result.statusBarHeight
   // 获取底部安全区域高度（全面屏手机）
   if (safeArea && height && autoScreenHeight) {
     result.bottomSafeHeight = autoScreenHeight - height - statusBarHeight
